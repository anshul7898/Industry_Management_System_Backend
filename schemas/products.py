...
# Valid dropdown options - UPDATED to match database values
VALID_BAG_MATERIALS = {
    'Cotton Canvas', 'Jute', 'Organic Cotton', 'Recycled Polyester', 'Linen', 'Cotton Blend'
}

VALID_SHEET_COLORS = {
    'Natural White', 'Off White', 'Cream', 'Brown', 'Kraft', 'Maroon', 'Beige'
}

VALID_BORDER_COLORS = {
    'Black', 'Brown', 'Navy Blue', 'White', 'Gray', 'Red', 'Gold', 'Maroon'
}
...
VALID_HANDLE_COLORS = {
    'Beige', 'Black', 'Brown', 'White', 'Navy Blue', 'Red', 'Gold', 'Maroon', 'Cream'
}

VALID_COLORS = {
    'White', 'Black', 'Cream', 'Brown', 'Navy Blue', 'Red', 'Green', 'Gray', 'Maroon', 'Gold'
}
...
class BaseProductModel(BaseModel):
    ...
    def validate_bag_material(cls, v):
        ...
        if v.strip() not in VALID_BAG_MATERIALS:
            raise ValueError(
                f"Invalid bag material '{v}'. Must be one of: {', '.join(sorted(VALID_BAG_MATERIALS))}"
            )
        return v.strip()
    ...
    def validate_sheet_gsm(cls, v):
        ...
        if v not in VALID_SHEET_GSMS:
            raise ValueError(
                f"Invalid sheet GSM '{v}'. Must be one of: {', '.join(map(str, sorted(VALID_SHEET_GSMS)))}"
            )
        return v
    ...
    def validate_sheet_color(cls, v):
        ...
        if v.strip() not in VALID_SHEET_COLORS:
            raise ValueError(
                f"Invalid sheet color '{v}'. Must be one of: {', '.join(sorted(VALID_SHEET_COLORS))}"
            )
        return v.strip()
    ...
    def validate_border_gsm(cls, v):
        ...
        if v not in VALID_BORDER_GSMS:
            raise ValueError(
                f"Invalid border GSM '{v}'. Must be one of: {', '.join(map(str, sorted(VALID_BORDER_GSMS)))}"
            )
        return v
    ...
    def validate_border_color(cls, v):
        ...
        if v.strip() not in VALID_BORDER_COLORS:
            raise ValueError(
                f"Invalid border color '{v}'. Must be one of: {', '.join(sorted(VALID_BORDER_COLORS))}"
            )
        return v.strip()
    ...
    def validate_handle_type(cls, v):
        ...
        if v.strip() not in VALID_HANDLE_TYPES:
            raise ValueError(
                f"Invalid handle type '{v}'. Must be one of: {', '.join(sorted(VALID_HANDLE_TYPES))}"
            )
        return v.strip()
    ...
    def validate_handle_color(cls, v):
        ...
        if v.strip() not in VALID_HANDLE_COLORS:
            raise ValueError(
                f"Invalid handle color '{v}'. Must be one of: {', '.join(sorted(VALID_HANDLE_COLORS))}"
            )
        return v.strip()
    ...
    def validate_handle_gsm(cls, v):
        ...
        if v not in VALID_HANDLE_GSMS:
            raise ValueError(
                f"Invalid handle GSM '{v}'. Must be one of: {', '.join(map(str, sorted(VALID_HANDLE_GSMS)))}"
            )
        return v
    ...
    def validate_printing_type(cls, v):
        ...
        if v.strip() not in VALID_PRINTING_TYPES:
            raise ValueError(
                f"Invalid printing type '{v}'. Must be one of: {', '.join(sorted(VALID_PRINTING_TYPES))}"
            )
        return v.strip()
    ...
    def validate_print_color(cls, v):
        ...
        if v.strip() not in VALID_PRINT_COLORS:
            raise ValueError(
                f"Invalid print color '{v}'. Must be one of: {', '.join(sorted(VALID_PRINT_COLORS))}"
            )
        return v.strip()
